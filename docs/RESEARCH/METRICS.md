...
| GNN — **engine-level** decision test (E2, wired via `refine_graph_costs`, trained checkpoint) | changes the Pareto top-fix in **0/60** real-CVE nets (moves per-edge success-prob magnitude mean 0.032 / max 0.347 but not the decision — the B1–B8 pattern) → **exploratory, default-off** | `E2_GNN_ROLE.md` |
...
**Classifier role (E1, decided 2026-06-15):** the classifier is **not in the NAMOA\*/Pareto path**
(confirmed by the import graph — only `/api/classify` and its trainer use it; the engine ranks from
EPSS/KEV/CVSS-vector). Decision = **KEEP, narrowly justified**: (1) a standalone analyst-triage tool
(0.729 F1), and (2) a coarse impact *fallback* (`severity_to_impact`) for the genuine no-CVSS case
(3b Qualys/OpenVAS/nmap emit no CVSS; 3a measured ~7.7% of recent CVEs un-scored). It never decides
which fix wins, the fallback is flagged heuristic, and the engine stays decoupled from the 266 MB
model. See `E1_CLASSIFIER_ROLE.md`.
...
| **B4** — time-to-exploit proxy | **Construct validity PASSES** (time monotone in CVSS Attack-Vector N<A<L<P & Complexity L<H; KEV 2.0× speed-up). **External validity NOW CORROBORATED (2026-06-16)** via CISA **KEV add-dates**: over **155** CVEs in KEV∩NVD-cache, `Spearman(proxy_time, disclosure→known-exploited window) = +0.263 [+0.11, +0.41]` — **CI excludes 0** (vs the original narrow zero-KEV sample's +0.02 straddling 0). The proxy is positively & significantly correlated with the real exposure window. **Honest confound:** `dateAdded` is CISA's *cataloguing* date (program launched 2021-11-03, bulk-added old CVEs → median window ~3,303 d, 0 post-launch), so it's a proxy-for-a-proxy; Metasploit/ExploitDB module-date grounding remains the gold standard. | `B4_EXTERNAL_GROUNDING.md`, `B4_B5_TIME_CRITICALITY.md` |
...
≥ 81.9% (n=60+); B4's *original* narrow-sample Spearman CI **[−0.2, +0.2]** straddled zero — but the
**KEV-add-date external grounding (2026-06-16) gives +0.263 [+0.11, +0.41], CI excluding 0** (n=155),
so B4 is now externally corroborated (modest, confounded by CISA cataloguing lag — see
`B4_EXTERNAL_GROUNDING.md`). Full table in `A5_STATISTICAL_RIGOR.md`. **Phase 1 (core-math soundness) is complete.**
...
| Item | Finding | Source |
|---|---|---|
| **A2 + A4** — neutral base-rate & stronger baselines | Re-ran the Phase-C oracle-recovery comparison with **real-CVE-sampled** nets (real EPSS/KEV) and **4 baselines** (CVSS, EPSS, risk = EPSS×CVSS, MulVAL-style reachability-filtered) on a **stacked**, an **un-stacked (neutral)**, and (optional follow-up) a **fully-real-CVSS** generator. **Pareto recovers ~84.7% of oracle reduction on stacked [79.2,89.7] & neutral [79.1,89.6], and 86.8% [81.1,91.6] on the fully-real-CVSS distribution** vs **~33–37% (synthetic CVSS) / ~35–46% (real CVSS)** for the baselines; Pareto ≥ each baseline ~88–96%. **The advantage is NOT a stacking OR synthetic-CVSS artifact** — un-stacking barely moves it, and going to fully-real CVSS *strengthens* the EPSS-based baselines a little (risk/EPSS → ~44–46%, since real CVSS correlates with severity) yet Pareto still recovers ~87% with a **non-overlapping CI** ([81.1,91.6] vs EPSS [38.2,53.8]); the edge is path/choke-point awareness (even reachability-aware MulVAL-style only gets ~35%). **Honest caveats:** the metric (reachability reduction) is aligned with what Pareto optimizes (oracle is method-independent, so the recovery is still meaningful); the REAL pool is the local NVD-cache∩EPSS intersection (3,200 CVEs), not all 340 k; one synthetic topology family (real-topology = 3c testbed + future external data). | `A2_A4_BASELINES.md` |
...
## 9. Modeling scope (Phase 5, in progress)
| Item | Finding | Source |
|---|---|---|
| **C1** — identity/credential/AD modeling (+ ATT&CK on edges) | New `core/identity_graph.py` models **credential** lateral movement (the biggest scope gap — none of it is a CVE) on the same canonical `AttackGraph`. Transitions are **MITRE ATT&CK techniques** (id + tactic on the `ExploitNode` and the edge). On the canonical AD kill-chain scenario (phished workstation → file server → app/SQL → Domain Controller goal), NAMOA\* recovers **2 Pareto credential paths** to Domain Admin: (1) Phish **T1566.001** → PtH **T1550.002** → RDP **T1021.001** (time 11.9, success 0.195), (2) Phish → PtH → Kerberoast **T1558.003** → DCSync **T1003.006** (time 14.0, **success 0.246**) — a faster-louder vs slower-higher-success tradeoff the engine keeps both of. **Exit criterion (an AD/credential lateral path in a testbed scenario) met.** **Honest caveat:** credential-technique costs are **heuristic** (no per-technique data feed), flagged `heuristic=True`/`data_grounded=False` on every edge — the contribution is the modeling capability + ATT&CK provenance, not a grounded credential probability. | `C1_IDENTITY_AD.md` |
| **C2** — cloud IAM / permission-lateral-movement modeling (+ ATT&CK on edges) | New `core/cloud_iam_graph.py` models the **cloud control-plane** gap — IAM permission abuse (leaked key → workload → IMDS credential theft → `AssumeRole`/`CreatePolicyVersion` to admin), none of which is a CVE — on the same canonical `AttackGraph` with cloud **ATT&CK techniques** on each edge (reuses C1's `Technique`). On the AWS privesc scenario (low-priv IAM user → EC2 instance → CI/CD role → Account Admin goal), NAMOA\* recovers **2 Pareto cloud-privesc paths**: (1) **T1078.004** → **T1651** → **T1548.005** direct elevation (time 11.9, success 0.195, fast/loud), (2) **T1078.004** → **T1651** → **T1552.005** IMDS theft → **T1098.003** Additional Cloud Roles (time 14.0, **success 0.246**, slower/higher-success). Cross-cloud (Azure Managed-Identity/PIM, GCP metadata-server/SA-impersonation) noted — the ATT&CK IDs are provider-agnostic. **Same honest caveat as C1:** cloud IAM technique costs are **heuristic** (`heuristic=True`/`data_grounded=False`); the grounded part is the *structure* (recoverable from an IAM-policy graph), the contribution is modeling + ATT&CK provenance. | `C2_CLOUD_IAM.md` |

| **C3** — misconfiguration (non-CVE weakness) modeling (+ CWE on edges) | New `core/misconfig_graph.py` models the **non-CVE weakness** gap — default/weak credentials, services exposed with no auth, world-readable shares, secrets in backups — on the same canonical `AttackGraph` with **CWE ids** (and ATT&CK ids where they apply) on each edge. On the misconfig breach scenario (DMZ web → app → backup → database goal, **no CVE anywhere**), NAMOA\* recovers **2 Pareto weakness chains**: (1) **CWE-798** default creds → **CWE-306** exposed no-auth → **CWE-306** DB no-auth (time 9.4, success 0.390, fast/loud), (2) **CWE-798** → **CWE-306** → **CWE-732** world-readable share → **CWE-522** secrets-in-backup (time 10.5, **success 0.458**, slower/higher-success). **Honest caveat:** misconfig costs are **heuristic** (`heuristic=True`/`data_grounded=False`); the nuance specific to misconfigs is that success priors are deliberately **high** (default creds ≈ 0.90) because the gating question is *presence* (what a config scanner reports), not exploitability — the grounded part is the *structure*, the contribution is modeling + CWE provenance. | `C3_MISCONFIG.md` |

| **C4** — "model, not validator" scoping + safe evidence grader | Explicit scoping statement: CTPPO is a prioritization/planning **model**, not a BAS **validator** — it computes the exact Pareto front and the highest-value fix, it does **not** fire exploits across an estate (the one empirical validation is the sandboxed 3c testbed). Plus `evaluation/path_validator.py`, a **safe, non-firing** evidence grader that classifies each path edge into a tier (`live_exploited`/`kev`/`high_epss`/`data_grounded`/`heuristic`) and reports a per-path **grounded fraction**. **Measured:** C1 AD & C3 misconfig paths grade **0% grounded (heuristic-only)**; the 3c live-testbed path grades **50% (mixed)** — entry CVE `live_exploited`, web→app lateral pivot `heuristic` (the by-construction prior the 3c doc flagged). Operationalizes the boundary: the grader shows, per recommendation, how much rests on data vs priors, without executing anything. | `C4_MODEL_NOT_VALIDATOR.md` |

**ML-role honesty (E1–E3, done 2026-06-15):** E1 — the severity classifier is **not in the Pareto
path** (import graph); decision = KEEP, narrowly justified (analyst tool + no-CVSS impact fallback),
`E1_CLASSIFIER_ROLE.md`. E2 — the GNN is **exploratory/default-off**: wired into the engine it
changes the Pareto top-fix in **0/60** real-CVE nets (moves magnitude not decision); its one
measured lift is topology (PIGNN +0.07 AUC), `E2_GNN_ROLE.md`. E3 — leakage/circularity audit:
severity split **measured 0 exact / 0 near-dup overlap** (240 real CVEs), GNN synthetic label is an
honest *recoverability* test, all splits graph-level/leakage-free, `E3_ML_LEAKAGE_AUDIT.md`.

**Phase 5 (modeling scope) is complete: C1 identity/AD · C2 cloud IAM · C3 misconfiguration ·
C4 model-not-validator + evidence grader · E1/E2/E3 ML-role honesty.**

## 10. Realtime product UX (Phase 6, in progress)
| Item | Finding | Source |
|---|---|---|
| **What-if remediation simulator** (surfaces the D4 exact-incremental engine) | `POST /api/attack-paths/whatif` + a frontend `WhatIfPanel` let an operator simulate patching CVE(s) and see the before/after Pareto front + reachability reduction. The D4 skip rule is exposed honestly: patching an **off-front** CVE returns `skipped_recompute=True` (the front is **provably unchanged**, no re-search); an **on-front** patch recomputes the exact front and reports the reachability it removes. Verified end-to-end (`tests/api/test_whatif_api.py`): off-front patch → 0 reduction / unchanged paths; on-front chain patch → reachability strictly drops. Frontend typecheck + build green. | `PHASE6_WHATIF.md` |
| **Per-path uncertainty bands** (B1/B2 made operational) | Every `/api/attack-paths/*` path now reports reachability as a **range** `[∏ pᵢ, min pᵢ]` (independence lower bound = the engine's point value; comonotone upper bound = the weakest edge) + a width factor, shown in the frontend `PathList`. **Measured:** the sample multi-host path's band is **[0.0003, 0.0561] (×185 wide)** — independence severely under-states reachability under correlation, exactly B2's finding. **Tested:** the independence bound == the engine's reported success (consistency); the band never inverts. Annotation only — no Pareto decision changes. | `PHASE6_UNCERTAINTY.md` |
| **SIEM/EDR/ticketing hooks** (G2) | `integrations/exporters.py` + `POST /api/integrations/export` format CTPPO findings into **ECS** (Elastic Common Schema) events, a **CEF** line, or a **Jira/ServiceNow-mappable ticket**, with an optional webhook dispatcher. The recommended choke-point fix + its reachability reduction (via the D4 `whatif_front`) ride along. **Honest scope:** formats + optionally POSTs; with no `webhook_url`, delivery is a `delivered=False` no-op (a real Splunk/Jira connector needs the operator's endpoint/creds — not faked). A CEF structural-pipe escaping bug was caught and fixed. 9 tests. | `PHASE6_INTEGRATIONS.md` |
