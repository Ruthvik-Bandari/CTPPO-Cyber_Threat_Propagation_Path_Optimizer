...
class WebsiteSecurityAnalyzer:
    ...
    def _create_exploit_cost(self, vuln: VulnerabilityFinding) -> EdgeCostVector:
        """Create cost vector for exploitation"""
        cost = EdgeCostVector.create_default()
        
        # Higher severity = easier to exploit
        time_hours = 2.0 + (5 - vuln.severity.value) * 1.0
        success_prob = 0.5 + (vuln.severity.value * 0.1)
        impact_score = vuln.severity.value * 2
        
        cost.components[CostType.TIME_TO_EXPLOIT] = create_time_cost(time_hours, time_hours * 0.3)
        cost.components[CostType.SUCCESS_PROBABILITY] = create_probability_cost(success_prob)
        cost.components[CostType.BUSINESS_IMPACT] = create_impact_cost(
            impact_score * 0.5, impact_score, impact_score * 1.5
        )
        return cost
        ...
        if pareto_paths:
            ...
            recommendations.append({
                ...
                "description": f"Found {len(pareto_paths)} potential attack paths. "
                              f"Fastest attack can complete in {fastest_path[1].values[0]:.1f} hours." if hasattr(fastest_path[1], 'values') else "Attack paths detected.",
