...
class SeverityClassifier(nn.Module):
    ...
    def __init__(
        ...
        self.classifier = nn.Sequential(
            ...
            nn.Linear(256, 128),
            nn.ReLU(),
            nn.Dropout(dropout_rate * 0.5),  # Lighter dropout before final layer
...
class SeverityTrainer:
    ...
    def __init__(
        ...
        # Class weights to handle imbalance (LOW, MEDIUM, HIGH, CRITICAL)
        # Higher weight = model pays more attention to that class
        if class_weights is None:
            # Based on inverse frequency: LOW(4%), MEDIUM(51%), HIGH(34%), CRITICAL(11%)
            class_weights = [8.0, 1.0, 1.5, 4.0]
        
        weights_tensor = torch.tensor(class_weights, dtype=torch.float).to(device)
        self.criterion = nn.CrossEntropyLoss(weight=weights_tensor, label_smoothing=0.1)
    ...
    def train(
        ...
        early_stopping_patience: int = 2  # Reduced from 3 for faster stopping
    ) -> Dict:
        """
        Train the model.
        ...
        # Initialize optimizer with higher weight decay for regularization
